...
const createRequest = async (input, callback) => {
  ...
  const MAINNET_LOG_CHUNKS = 100000;
  ...
  const accessControlledOffchainAggregatorABI =
    config.accessControlledOffchainAggregatorABI;
  ...
  const web3 = new Web3(
    new Web3.providers.HttpProvider(RPC_URL, { timeout: 10e3 })
  );
  ...
  try {
    ...
    if (aggregators.length == 0) {
      ...
      aggreagatorBeingSearchedAddress = aggregators[0]; //only 1 aggregator, this is the one we need to search
      ...
      for (let i = aggregators.length - 1; i >= 0; i--) {
        ...
        var tempAggregator = await new web3.eth.Contract(
          accessControlledAggregatorABI,
          aggregators[i]
        );
        ...
        let earliestTimestamp = await aggreagatorBeingSearched.methods
          .getTimestamp(1)
          .call();
        log.verbose(
          new Date().toISOString() +
            ": earliest timestamp for aggreagator " +
            aggreagatorBeingSearchedAddress +
            " : " +
            earliestTimestamp
        );
        if (earliestTimestamp < specifiedDateTime) {
          //if this aggregator first round timestamp < the date we're searching for, this is the correct one that will contain our price data
          aggregatorAddress = aggreagatorBeingSearchedAddress;
          log.verbose(
            new Date().toISOString() +
              ": found aggreagator contract that matches criteria: " +
              aggreagatorBeingSearchedAddress
          );
    ...
    log.verbose(
      new Date().toISOString() + ": number of rounds found: " + rounds.length
    );
    log.verbose(new Date().toISOString() + ": starting binary search");
    var binSearchResult = await binarySearch(
      filterTopic,
      rounds,
      Math.floor((rounds.length - 1) / 2)
    );
    var foundRounds = binSearchResult;
    log.verbose(
      new Date().toISOString() +
        ": binary search complete. Results: " +
        JSON.stringify(foundRounds)
    );
    ...
    if (foundRounds.length > 0) {
      ...
      var sortedArray = foundRounds.sort(
        (a, b) => a.updatedTime - b.updatedTime
      );

      //now that we have an ordered array, we need to find the greatest updatedAt that is <= specifiedDateTime
      var sortedArray = sortedArray.filter((x) => {
        return x.updatedTime <= specifiedDateTime;
      });

      if (sortedArray.length == 0) {
        throw "no results found earlier than specified date";
      }
      var targetRound = Math.max.apply(
        Math,
        sortedArray.map((o) => o.updatedTime)
      );

      var roundAnswer = foundRounds.find(
        (o) => o.updatedTime === JSON.stringify(targetRound)
      );
      ...
      */
      ...
      //binary search found no results
  ...
  async function binarySearch(filterTopic, roundArray, midParam) {
    ...
    while (start <= end) {
      ...
      log.verbose(
        new Date().toISOString() +
          ": performing binary search on start: " +
          start +
          " mid: " +
          mid +
          " end: " +
      ...
      if (filterTopic == NEW_ROUND_TOPIC) {
        historicalRoundData = await aggreagatorBeingSearched.methods
          .getRoundData(roundArray[mid])
          .call();
        midDateTime = historicalRoundData.updatedAt;
        midPrice = historicalRoundData.answer;
      } else {
        midDateTime = await aggreagatorBeingSearched.methods
          .getTimestamp(roundArray[mid])
          .call();
        midPrice = await aggreagatorBeingSearched.methods
          .getAnswer(roundArray[mid])
          .call();
      }
      ...
      if (midDateTime == 0) {
        log.verbose(
          new Date().toISOString() +
            ": got a bad result for round " +
            roundArray[mid] +
            ", removing it and doing recursion"
        );
        ...
        while (zeroAfterTest == 0) {
          zeroAfterTest = await aggreagatorBeingSearched.methods
            .getTimestamp(roundArray[mid + zerosAfter])
            .call();
          if (zeroAfterTest == 0) {
            zerosAfter += 1;
          }
        ...
        while (zeroBeforeTest == 0) {
          zeroBeforeTest = await aggreagatorBeingSearched.methods
            .getTimestamp(roundArray[mid - zerosBefore])
            .call();
          if (zeroBeforeTest == 0) {
            zerosBefore += 1;
          }
        ...
        log.verbose(
          new Date().toISOString() +
            ": done stripping out 0 results, new array length: " +
            roundArray.length
        );
        return binarySearch(filterTopic, roundArray);
      } else {
        //now check to see if answer dateTime is within 2 hours of the search param. If so add it to a new array, we'll go through all found values later and work out which is the right one
